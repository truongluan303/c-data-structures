 ...
 * Usage:
 *
 * tester_init();                  -- Set up testing system.
 * tester_report(FILE *, "name");  -- Print report.
 * tester_result();                -- Returns # of failed tests.
 *
 * TRY { code } ENDTRY;
 *
...
/**
 * @brief Set a time limit t for TRY, TEST, TEST_ASSERT etc.
...
/**
 * @brief  Wraps code to catch seg faults, illegal instructions, etc. May not
 *         be nested.
 *         Prints a warning if a signal is caught.
 *         To enforce a maximum time, set alarm before entering.
 *
 * @example TRY { code } ENDTRY;
 */
#define TRY                                                                    \
...
#define ENDTRY else {                                                          \
        fprintf(stderr, "%s:%d: %s (signal %d)\n",                             \
                __FILE__,                                                      \
                __LINE__,                                                      \
                strsignal(TesterData.setjmp_return),                           \
...
/**
 * @brief Evaluates expr (which should yield an integer value) inside a TRY.
 *        Prints a warning if evaluating expr causes a fault or returns a value
 *        not equal to expected_value.
 */
#define TEST(expr, expected_value)                                             \
    ...
    TRY {                                                                      \
        ...
        if (TesterData.expr_value != expected_value) {                         \
            fprintf(stderr,                                                    \
                    "%s:%d: TEST FAILED: %s => %d but expected %d\n",          \
                    __FILE__,                                                  \
                    __LINE__,                                                  \
                    __STRING(expr),                                            \
                    TesterData.expr_value,                                     \
                    expected_value);                                           \
        } else {                                                               \
            TesterData.errors--;                                               \
        }                                                                      \
    ...
    if(TesterData.try_failed) {                                                \
        fprintf(stderr,                                                        \
                "%s:%d: TEST FAILED: %s caught signal\n",                      \
                __FILE__,                                                      \
                __LINE__,                                                      \
                __STRING(expr));                                               \
    }
...
/**
 * @brief Equivalent to TEST(!(expr), 0)
 */
#define TEST_ASSERT(expr) TEST((expr) != 0, 1)

/**
 * @brief Causes your own failure.
 *
 * @example ```
 * TRY {
 *     x = 1;
